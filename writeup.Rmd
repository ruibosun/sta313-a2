...
ggplot(hour_plot, aes(x = OCC_HOUR, fill=season)) +
  geom_bar() +
  scale_x_discrete(drop = FALSE) + # Include all levels, even if some have no data
  coord_polar(start = 0 - radians_to_rotate)+
  theme_minimal()+
  theme(
    axis.text.y = element_blank(),  # This will remove the radial axis labels
    panel.grid.major.y = element_blank(),  # This will remove the major grid lines (optional)
    panel.grid.minor.y = element_blank()  # This will remove the minor grid lines (optional)
  )
