...
## random plot (for interative plot)
```{r}
library(plotly)

fig <- bike_raw %>%
  group_by(BIKE_C, BIKE_COLOUR) %>%
  summarise(Count = n(), .groups = 'drop') %>%
  ggplot(aes(x = BIKE_TYPE, y = Count, color = BIKE_COLOUR)) +
  geom_bar() +
  labs(title = "Interactive View: Bike Thefts by Type and Color",
       x = "Bike Type",
       y = "Number of Thefts")
